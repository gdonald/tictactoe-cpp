...
int Board::getWinner() {
  for (auto col = 0; col < 3; col++)
    if(moves[0][col] != EMPTY && moves[0][col] == moves[1][col] && moves[0][col] == moves[2][col]) { return moves[0][col]; }

  for (auto row = 0; row < 3; row++)
    if(moves[row][0] != EMPTY && moves[row][0] == moves[row][1] && moves[row][0] == moves[row][2]) { return moves[row][0]; }
